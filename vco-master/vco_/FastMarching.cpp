	...
	cv::Mat *o_matD, double** o_S)
{
	...
	//Q = i_Q;
	//PD = i_PD;
	D = new double[i_Ww*i_Wh];
	S = new double[i_Ww*i_Wh];
	Q = new double[i_Ww*i_Wh];
	PD = new double[i_Ww*i_Wh];

	for (int y = 0; y < i_Wh; y++)
	for (int x = 0; x < i_Ww; x++)
	{
		W[y*i_Ww + x] = i_W.at<double>(y, x);
		H[y*i_Ww + x] = 0;
		D[y*i_Ww + x] = 1e9;
		S[y*i_Ww + x] = 1;
		Q[y*i_Ww + x] = -1;
	...
	cv::Mat matD(i_Wh, i_Ww, CV_64FC1);
	for (int y = 0; y < i_Wh; y++)
	for (int x = 0; x < i_Ww; x++)
	{
		matD.at<double>(y, x) = D[y*i_Ww + x];
	}
