...
void frangi2_eig2image(const Mat &Dxx, const Mat &Dxy, const Mat &Dyy, Mat &lambda1, Mat &lambda2, Mat &Ix, Mat &Iy, cv::Mat &o_mul1, cv::Mat &o_mul2){
...
void frangi2d(const Mat &src, Mat &maxVals, Mat &whatScale, Mat &outAngles, frangi2d_opts_t opts){
	...
	for (float sigma = opts.sigma_start; sigma <= opts.sigma_end; sigma += opts.sigma_step){
		...
		Rb = Rb.mul(Rb);
		cv::Mat S2 = mul1.mul(mul1) + mul2.mul(mul2);
		...
		cv::Mat Ifiltered = (Mat::ones(src.rows, src.cols, src.type()) - tmp2);
		//if (opts.BlackWhite){
